  ...
  return (
    <footer className="bg-umbi-bg pt-[var(--footer-pt)] pb-[var(--footer-pb)] px-6 md:px-[var(--footer-px-md)]">
      <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-8 mb-8">
        <Link href="/" className="relative w-[177px] h-[51px] block shrink-0 cursor-pointer">
          <Image
      ...
      <div className="flex flex-col md:flex-row justify-between gap-8 font-serif text-ui text-umbi-dark leading-relaxed">
        ...
        <div className="flex flex-col gap-1">
          <Link href="/#slik-fungerer-det" className="hover:opacity-60 transition-opacity cursor-pointer">Slik fungerer det</Link>
          <Link href="/reise" className="hover:opacity-60 transition-opacity cursor-pointer">Vår reise</Link>
          <Link href="/design" className="hover:opacity-60 transition-opacity cursor-pointer">Design din Umbi Uno</Link>
        </div>
        <div>
          <p>hei@umbi.no</p>
          <p className="opacity-50 text-sm mt-2">© {new Date().getFullYear()} Umbi</p>
        </div>
      </div>
    </footer>
  );
